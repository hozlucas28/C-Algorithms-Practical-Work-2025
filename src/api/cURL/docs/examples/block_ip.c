...
enum connection_filter_t { CONNECTION_FILTER_BLACKLIST, CONNECTION_FILTER_WHITELIST };
...
static int ip_match(struct ip *ip, void *netaddr) {
    ...
    if (tailbits) {
        ...
        if ((*x & tailmask) != (*y & tailmask)) return FALSE;
    }
        ...
        if (cinaddr) {
...
#ifdef AF_INET6
            int mapped =
                !filter->ipv6_v6only && is_ipv4_mapped_ipv6_address(address->family, cinaddr);
#endif

            for (ip = filter->list; ip; ip = ip->next) {
                if (ip->family == address->family && ip_match(ip, cinaddr)) break;
#ifdef AF_INET6
            ...
            if (ip && filter->type == CONNECTION_FILTER_BLACKLIST) {
                ...
                if (filter->verbose) {
                    char buf[128] = {0};
                    inet_ntop(address->family, cinaddr, buf, sizeof(buf));
                    fprintf(stderr, "* Rejecting IP %s due to missing whitelist entry.\n", buf);
                }
                return CURL_SOCKET_BAD;
            }
        }
...
int main(void) {
    ...
    if (curl_global_init(CURL_GLOBAL_DEFAULT)) exit(1);
